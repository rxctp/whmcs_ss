...
set @query = if ((select count(*) from information_schema.columns where table_schema=database() and table_name='tblticketdepartments' and column_name='feedback_request') = 0, 'ALTER TABLE tblticketdepartments ADD `feedback_request` tinyint(1) NOT NULL DEFAULT \'0\'', 'select 0');
...
set @query = if ((select count(*) from information_schema.columns where table_schema=database() and table_name='tbldomainpricing' and column_name='group') = 0, 'ALTER TABLE tbldomainpricing ADD `group` VARCHAR(5) COLLATE utf8_unicode_ci NOT NULL DEFAULT \'none\'', 'select 0');
prepare statement from @query;
execute statement;
deallocate prepare statement;
